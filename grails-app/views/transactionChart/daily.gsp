  ...
  <title>Transaction Chart</title>
    ...
    <script type="text/javascript">
        ...
        function initECFChart(){
            ...
            jQuery.post('${request.contextPath}/transactionChart/getTransChartData',{dateBegin:jQuery('#transChartBeginDate').val(), dateEnd:jQuery('#transChartEndDate').val()},function(data){
                totalSales = new Array();
                totalItem  = new Array();

                jQuery.each(data.data,function(idx, val){
                    totalSales.push([val.transactionDate, parseFloat(val.totalSales)]);

                    //alert(totalSales[idx][0] + ', ' + totalSales[idx][1]);
                });

                $.jqplot._noToImageButton = true;

                var dailyPlot = $.jqplot("TransactionChart", [totalSales], {
                    seriesColors: ["rgba(78, 135, 255, 0.6)"],
                    title: 'Sales Transaction Chart',
                    highlighter: {
                        show: true,
                        sizeAdjust: 1,
                        tooltipOffset: 9
